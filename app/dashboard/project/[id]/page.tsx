    ...
    <div className="flex h-screen bg-black text-white font-sans overflow-hidden">
      <div className="fixed top-0 left-64 right-0 h-16 bg-black/80 backdrop-blur-md border-b border-white/10 flex items-center justify-between px-8 z-10">
          ...
          <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-1.5 border border-white/5"><Brain size={14} className="text-slate-500" /><select value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} className="bg-transparent text-xs font-bold text-slate-300 outline-none cursor-pointer max-w-[150px]">{availableModels.map((m: any) => <option key={m} value={m} className="bg-black text-white">{m}</option>)}</select></div>
              <div className="flex items-center gap-2 bg-white/5 rounded-lg px-3 py-1.5 border border-white/5"><AlignLeft size={14} className="text-slate-500" /><select value={selectedTone} onChange={(e) => setSelectedTone(e.target.value)} className="bg-transparent text-xs font-bold text-slate-300 outline-none cursor-pointer">{TONES.map(t => <option key={t} value={t} className="bg-black text-white">{t}</option>)}</select></div>
              {activeTab !== 'concept' && <button onClick={handleGenerateScript} disabled={generating} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 rounded-lg font-bold text-sm flex items-center gap-2 transition-all disabled:opacity-50 shadow-lg shadow-purple-900/20">{generating ? <Loader2 className="animate-spin" size={16}/> : <Sparkles size={16} />} <span>{scenes.length > 0 ? 'Regenerate' : 'Generate'}</span></button>}
          </div>
      </div>
              ...
              <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4">
                   {scenes.length === 0 && !generating && (
                       <div className="text-center py-20 border-2 border-dashed border-white/10 rounded-3xl bg-white/5 flex flex-col items-center justify-center">
                           ...
                           <button onClick={handleGenerateScript} className="px-8 py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl text-lg flex items-center gap-3 shadow-2xl"><Sparkles size={24} /> Generate Script</button>
                       </div>
                   )}
                   {generating && scenes.length === 0 && <div className="text-center py-20"><Loader2 className="animate-spin mx-auto text-purple-500 mb-4" size={48}/><p className="text-slate-400 animate-pulse">Writing your masterpiece...</p></div>}
                   {scenes.map((scene, i) => (
